...
// 내가 클릭한 순서에 맞는 .content 요소가 나타느게 만들기

// li 클릭이 아니라, 내부의 버튼을 클릭 버튼의 부모인 li 몇 번째인가?
// click(선택된 상태)/focus/hover 각각의 상황에 맞게 표현처리방법?
// 선택으로 인해 내용이 나타나면, 먼저 읽어주어야 하는 표현은 무엇인가?
...
// jQuery : selector.attr('속성명') -> 해당 속성의 값을 파악
// jQuery : selector.attr('속성명', '값') -> 해당 속성의 값을 처리
...
tabBtn.on('focus', function(e){
  ...
  // -> button으로 주지 않는 이유는 button은 li의 자식이므로 형제가 없어 무조건 첫번째가 된다. 그래서 부모 li의 순서를 파악한다. 
  var _i = _thisParent.index();

  var _contentEq = tabContent.eq( _i );
  contentSelect = _contentEq;

  // jQuery는 애니메이션 기능이 들어있는 메소드는 콜백기능이 있다.
...
// js : 선택자.addEventListner('keyup', function(event){});
