...
var b02LiLen = b02Li.length;
  // 202
ban_02_total.text( b02Li.length );
...
var countUp = 0;
  // 201
ban_02_now.text( countUp + 1 ); // 클릭 이전
...
b02Next.on('click', function(e){
  ...
  // 4.-------------------------
    ...
    ban_02_now.text( countUp + 1 ); // 클릭 이후
  // 2. ---------------------------------
...
b02Prev.on('click', function(e){
  ...
  console.log(countUp);
  // 201(클릭시마다 변하게 처리 / 작성위치 파악)
  ban_02_now.text( countUp + 1 ); // 73번째에 써도 됨
});// b02Prev.on('click' ...);
