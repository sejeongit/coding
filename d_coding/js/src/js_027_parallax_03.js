...
// 2. 비행기 이미지 상단으로 이동(현재 가지고있는 top값에서 변경)
// 3. 비행기 이미지가 상단에 닿으면, .logo가 나타나게 만들기 ( opacity )
// 3-1. 브라우저의 높이를 기준으로 이동한 값을 %로 계산 (이동값 / 기준 * 1) => 1을 100%
// 4. logo가 완전히 나타나면 #headBox를 나타나게 만들자!
// 5. #viewBox의 높이값을 수정(기존값은 너무 임의값이기에 조금 더 보기좋은 높이로 조정)
// 5-1. logo가 opacity값이 1이 되었을때! headBox의 offset().top + 화면의 높이값*2
...
// 기능 --------------------------------
var img_01Top = parseInt(coverImg_01.css('top')); // css 가져올땐 단위까지 가져오므로 parseInt
...
var scrollFn = function(){
  ...
  coverImg_02.css({left: coverImg_02_Left +'px', top: coverImg_02_Top + 'px'}); // 기본단위 px이므로 단위 안써도 먹음
