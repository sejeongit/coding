...
// 5. 기준이 화면의 하단이기에 : li의 위치값 + 브라우저의 높이값
// 5-1. 화면의 하단에서 1/3지점 위치에서 나타나게 하기 위해 수정 ( 브라우저 높이/3 )
// 6. 스크롤을 이동시 지정한 offset값 ( setOffset )과 비교하여 이보다 커지면 체크
// 7. 체크되는 곳에서 li에 act 클래스 이름 부여
// 8. 옵션: 반대의 경우에는 act 빼기
// 9. li요소 전부 동작
...
var setLiFn = function(scroll){
  ...
  for (; i<liLen ; i+=1){
    ...
    setOffset = liOffset - (winH/3 *2); // 5-1. 
