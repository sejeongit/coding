...
// .part가 위에서 얼만큼 떨어져 있는가?
var part = $('.part');
var partOffset = part.offset().top;
console.log('offset part:', partOffset);

// 화면 스크롤시 스크롤값이 얼만큼 움직였는가?
var win = $(window); // window가 원래 모니터 화면 전체이지만 jQuery에선 도큐먼트만 포함함
var winH = win.outerHeight(); // 브라우저 document의 보이는 화면 높이 만큼
...
win.on('scroll', function(){
  ...
  // % 계산방법 : 움직인값(winScroll) / 기준(브라우저높이) * 100
  // 소수점을 강제로 정수처리 ( 내림: parseInt(수치) )
  var move = winScroll - parOffset + winH;
  var perPart = parseInt( move / winH * 100 );

  console.log( perScroll, parPart );
  // console.log(move);
...
/*
  ...
  스크롤시 이동한 만큼에서 떨어져있는 만큼을 뺐을 때 실제로 기준이 상단이 아니기에 추가로 높이만큼을 추가
  이동한 식 = (브라우저.scrollTop() - 선택자.offset().top + $(window).height())
  
  위 이동한 식에서 기준을 나누고 100을 곱하면 하단에서부터 이동한만큼을 %로 확인
  이동식 / 브라우저높이 * 100
